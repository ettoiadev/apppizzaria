    ...
    const customersWithDetails = await Promise.all(
      (customers || []).map(async (customer: any) => {
        try {
          ...
          const totalSpent = ordersStats?.reduce((sum, order) => sum + parseFloat(order.total || '0'), 0) || 0
          const lastOrderAt = ordersStats?.length ? 
            ordersStats.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].created_at : 
            null
