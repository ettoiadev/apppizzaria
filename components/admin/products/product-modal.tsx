        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="price" type="number" step="0.01" min="0" value={formData.price || 0} onChange={(e) => handleFormChange('price', parseFloat(e.target.value) || 0)} required />
              </div>
              <div>
              ...
              <div className="space-y-2">
                {imagePreview ? (
                  <div className="relative inline-block">
                    ...
                    <Button type="button" variant="destructive" size="icon" onClick={removeImage} className="absolute -top-2 -right-2 w-6 h-6 rounded-full">
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                ) : (
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            {formData.sizes?.map((size, index) => (
              <div key={index} className="flex gap-2 items-center">
                <Input placeholder="Nome (Ex: Grande)" value={size.name} onChange={(e) => updateDynamicListItem('sizes', index, "name", e.target.value)} />
                <Input placeholder="Preço" type="number" step="0.01" min="0" value={size.price} onChange={(e) => updateDynamicListItem('sizes', index, "price", Number(e.target.value))} />
                <Button type="button" variant="ghost" size="icon" onClick={() => removeDynamicListItem('sizes', index)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            {formData.toppings?.map((topping, index) => (
              <div key={index} className="flex gap-2 items-center">
                <Input placeholder="Nome (Ex: Queijo Extra)" value={topping.name} onChange={(e) => updateDynamicListItem('toppings', index, "name", e.target.value)} />
                <Input placeholder="Preço" type="number" step="0.01" min="0" value={topping.price} onChange={(e) => updateDynamicListItem('toppings', index, "price", Number(e.target.value))} />
                <Button type="button" variant="ghost" size="icon" onClick={() => removeDynamicListItem('toppings', index)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
              </div>
            ))}
          </div>
